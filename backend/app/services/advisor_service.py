...
class InvestmentAdvisorService:
    ...
    def __init__(self):
        ...
        # Initialize Mistral "The Brain" (was Gemini — swapped, see chat notes)
        self.client = None
    ...
    ASSET_TICKERS = {
        ...
        "moderate": ["^BSESN", "^NSEI"],          # Sensex + Nifty 50 (fixed: NIFTY_50.NS isn't a real Yahoo ticker)
    ...
    def get_ai_brain_advice(self, context: Dict) -> str:
        """
        The 'Brain' of the engine: uses Mistral to synthesize market data
        and user profile into a cohesive strategy. (Was Gemini — the model
        name it used, gemini-1.5-flash, is deprecated/unavailable, which is
        why this was showing a raw 404 error in the UI.)
        """
        try:
            prompt = f"""
            ...
            - Use plain text only. Do not include any markdown syntax such as **bold**, __underline__, or lists with bullets.
