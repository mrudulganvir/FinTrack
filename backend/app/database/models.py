...
class Loan(Base):
    ...
    amount      = Column(Float(precision=2), nullable=False)
    interest_rate = Column(Float(precision=2), nullable=True)
...
class ZerodhaConnection(Base):
    ...
    kite_user_id = Column(String(50), nullable=True)   # Zerodha's own user id, e.g. "AB1234", handy for display
    login_time   = Column(DateTime, nullable=True)      # when this access_token was issued (Kite invalidates it ~7:30 AM IST next day)
    created_at   = Column(DateTime(timezone=True), server_default=func.now())
