...
def google_login(payload: GoogleTokenRequest, db: Session = Depends(get_db_connection)):
    if not GOOGLE_CLIENT_ID:
        raise HTTPException(
            status_code=500,
            detail="Google auth not configured on server — check GOOGLE_CLIENT_ID in .env"
        )

    try:
        # The verification happens here. It checks if the token was 
        # generated by the same GOOGLE_CLIENT_ID defined above.
        ...
        print(f"DEBUG: Google Verification ValueError: {e}")
        raise HTTPException(status_code=401, detail=f"Invalid Google token: {str(e)}")
    except Exception as e:
        print(f"DEBUG: Google Verification Unexpected Error: {e}")
