...
const Loans = () => {
  ...
  const [formData, setFormData] = useState({ person_name: '', amount: '', interest_rate: '', due_date: '', loan_type: 'borrowed' });
  ...
  const handleAddLoan = async (e) => {
    ...
    try {
      ...
      setFormData({ person_name: '', amount: '', interest_rate: '', due_date: '', loan_type: 'borrowed' });
    } catch (err) {
        ...
        {loading ? (
          ...
          loans.map(loan => (
            <div key={loan.id} className={`glass p-6 rounded-[2rem] border-glass-border relative overflow-hidden group ${loan.status === 'paid' ? 'opacity-60' : ''}`}>
              ...
              <h3 className="text-xl font-bold mb-1 truncate">{loan.person_name}</h3>
            ...
            <form onSubmit={handleAddLoan} className="space-y-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input type="text" required value={formData.person_name} onChange={(e) => setFormData({ ...formData, person_name: e.target.value })} placeholder="e.g. HDFC Bank, Uncle Joe" className="w-full bg-white/5 border border-white/10 rounded-xl py-3 px-4 focus:border-orange-500/50 outline-none text-sm" />
                </div>
